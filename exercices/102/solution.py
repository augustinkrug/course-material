...
def check_my_city(town):
    stations_nb = 0
    zip_code = []
    city = ""
    for i in range(len(velib)):
        if town in velib[i]['city']:
            stations_nb = stations_nb + 1
            zip_code.append(velib[i]['zip'])
            city = velib[i]['city']
    if stations_nb == 0:
        return print("Sorry! No station for your city has been found!")
    else:
        return print(
